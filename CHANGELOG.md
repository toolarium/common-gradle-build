...
- `defaults.gradle`: updated default env var names to Quarkus `KC_*` equivalents (all existing property names kept for backward compatibility): `KEYCLOAK_USER_FILE`→`KEYCLOAK_ADMIN`, `KEYCLOAK_PASSWORD_FILE`→`KEYCLOAK_ADMIN_PASSWORD`, `DB_VENDOR`→`KC_DB`, `DB_ADDR`→`KC_DB_URL_HOST`, `DB_PORT`→`KC_DB_URL_PORT`, `DB_DATABASE`→`KC_DB_URL_DATABASE`, `DB_SCHEMA`→`KC_DB_SCHEMA`, `DB_USER_FILE`→`KC_DB_USERNAME`, `DB_PASSWORD_FILE`→`KC_DB_PASSWORD`, `KEYCLOAK_LOGLEVEL`→`KC_LOG_LEVEL`, `PROXY_ADDRESS_FORWARDING`→`KC_PROXY_HEADERS`, `KEYCLOAK_FRONTEND_URL`→`KC_HOSTNAME`. ConfigMap key defaults updated: `db.addr`→`db.host`, `frontend.url`→`hostname.url`, `proxy.address.forwarding`→`proxy.headers`. `kubernetesIdmAddressForwarding` default changed `true`→`xforwarded`. Health probe path defaults changed from `${kubernetesIdmUrlPath}/health/*` to `/health/*` (management port, independent of context path).
- `defaults.gradle`: added new properties `kubernetesIdmManagementPort` (`9000`), `kubernetesIdmHttpEnabledEnvName`/`Value` (`KC_HTTP_ENABLED`/`true`), `kubernetesIdmHostnameStrictEnvName`/`Value` (`KC_HOSTNAME_STRICT`/`false`), `kubernetesIdmCacheEnvName`/`Value` (`KC_CACHE`/`ispn`), `kubernetesIdmCacheStackEnvName`/`Value` (`KC_CACHE_STACK`/`jdbc-ping`).
